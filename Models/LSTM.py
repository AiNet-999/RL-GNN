...
input_sequence = 3   # past 3 days as input like our proposed
...
for stock in top_stocks:
    ...
    model = Sequential([
        LSTM(64, activation='tanh', input_shape=(input_sequence, 1)),
        Dense(1)
    ])
...
print("\n=== Overall Average Metrics (LSTM + MinMaxScaler, 1-step Ahead) ===")
...
for stock in top_stocks:
    ...
    plt.plot(dates, preds, label='Predicted (LSTM, H=1)', linestyle='--', color='blue')
    plt.title(f"{stock} - LSTM Forecast (1-Step Ahead, Original Scale)")
    plt.xlabel("Date")
    plt.ylabel("Price")
    plt.legend()
    plt.show()
