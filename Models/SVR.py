...
# === Parameters ===
input_sequence = 3   # past 3 days as input same to propsoed model
...
for stock in top_stocks:
    ...
    train, test = series[:size], series[size - input_sequence:]  # keep overlap for continuity
...
print("\n=== Overall Average Metrics (SVR + MinMaxScaler, 1-step Ahead) ===")
...
for stock in top_stocks:
    ...
    plt.plot(dates, preds, label='Predicted (SVR, H=1)', linestyle='--', color='blue')
    plt.title(f"{stock} - SVR Forecast (1-Step Ahead, Original Scale)")
    plt.xlabel("Date")
    plt.ylabel("Price")
    plt.legend()
    plt.show()
