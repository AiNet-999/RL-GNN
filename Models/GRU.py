...
for stock in top_stocks:
    ...
    model = Sequential([
        GRU(64, activation='tanh', input_shape=(input_sequence, 1)),
        Dense(1)
    ])
...
print("\n=== Overall Average Metrics (GRU + MinMaxScaler, 1-step Ahead) ===")
...
for stock in top_stocks:
    ...
    plt.plot(dates, preds, label='Predicted (GRU, H=1)', linestyle='--', color='blue')
    plt.title(f"{stock} - GRU Forecast (1-Step Ahead, Original Scale)")
    plt.xlabel("Date")
    plt.ylabel("Price")
    plt.legend()
    plt.show()