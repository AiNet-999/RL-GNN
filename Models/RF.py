...
for stock in top_stocks:
    print(f"\nTraining Random Forest for stock: {stock}")
...
print("\n=== Overall Average Metrics (Random Forest + MinMaxScaler, 1-step Ahead) ===")
...
for stock in top_stocks:
    ...
    plt.plot(dates, preds, label='Predicted (Random Forest, H=1)', linestyle='--', color='blue')
    plt.title(f"{stock} - Random Forest Forecast (1-Step Ahead, Original Scale)")
    plt.xlabel("Date")
    plt.ylabel("Price")
    plt.legend()
    plt.show()
