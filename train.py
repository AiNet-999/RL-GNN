...
closing_prices = (
    pd.read_csv('/kaggle/input/datasets/fizza1212/spdata/SP500_Closing_Prices-NEW - Copy.csv', header=None)
    .ffill()
    .bfill()
)

adjacency_matrix_sideways = pd.read_csv('/kaggle/input/datasets/fizza1212/spdata/correlation_matrix_sideways.csv', header=None).to_numpy()
adjacency_matrix_bear = pd.read_csv('/kaggle/input/datasets/fizza1212/spdata/correlation_matrix_bear.csv', header=None).to_numpy()
adjacency_matrix_bull = pd.read_csv('/kaggle/input/datasets/fizza1212/spdata/correlation_matrix_bull.csv', header=None).to_numpy()
adjacency_matrix_std = pd.read_csv('/kaggle/input/datasets/fizza1212/spdata/adjacency_matrix (1).csv', header=None).to_numpy()
num_stocks = 50  
num_stocks = 50
...
class GraphConv(layers.Layer):
    def __init__(self, in_feat, out_feat, graph_info,sta,weig, static_adjacency_matrix, aggregation_type="mean", combination_type="concat", activation=None, **kwargs):
...
class LSTMGC(layers.Layer):
    def __init__(self, in_feat, out_feat, lstm_units, input_seq_len, output_seq_len, graph_info1,graph_info2,graph_info3, static_adjacency_matrix, graph_conv_params=None, **kwargs):
        super().__init__(**kwargs)
        if graph_conv_params is None:
            graph_conv_params = {"aggregation_type": "mean", "combination_type": "concat", "activation": None}
        self.graph_conv1 = GraphConv(in_feat, out_feat, graph_info1,1,0, static_adjacency_matrix, **graph_conv_params)
        self.graph_conv2 = GraphConv(in_feat, out_feat, graph_info2,1,0,static_adjacency_matrix, **graph_conv_params)
        self.graph_conv3 = GraphConv(in_feat, out_feat, graph_info3,1,0, static_adjacency_matrix,**graph_conv_params)
        self.graph_conv4 = GraphConv(in_feat, out_feat, graph_info1,0,0.5, static_adjacency_matrix,**graph_conv_params)
        self.lstm1 = layers.LSTM(lstm_units, return_sequences=False)
        self.dense1 = layers.Dense(output_seq_len)
        self.lstm2 = layers.LSTM(lstm_units, return_sequences=False)
        self.dense2 = layers.Dense(output_seq_len)
    ...
    def call(self, inputs):
        ...
        shape= tf.shape(gcn_out1)
        num_nodes, batch_size, input_seq_len, out_feat = shape[0], shape[1], shape[2], shape[3]
        gcn_out1 = tf.reshape(gcn_out1, (batch_size * num_nodes, input_seq_len, out_feat))
        lstm_out1 = self.lstm1(gcn_out1)
        dense_output1 = self.dense1(lstm_out1)
        gcn_out2 = self.graph_conv2(inputs)
        shape = tf.shape(gcn_out2)
        num_nodes, batch_size, input_seq_len, out_feat = shape[0], shape[1], shape[2], shape[3]
        gcn_out2 = tf.reshape(gcn_out2, (batch_size * num_nodes, input_seq_len, out_feat))
        lstm_out2 = self.lstm2(gcn_out2)
        dense_output2 = self.dense2(lstm_out2)
        ...
        dense_output=  dense_output1 + dense_output2 +dense_output3+  dense_output4
        output = tf.reshape(output, (num_nodes, batch_size, self.output_seq_len))
...
adjacency_matrix_sideways= compute_adjacency_matrix(adjacency_matrix_sideways, epsilon=0.5)
node_indices, neighbor_indices = np.where(adjacency_matrix_sideways == 1)
graph1 = GraphInfo(edges=(node_indices.tolist(), neighbor_indices.tolist()), num_nodes=adjacency_matrix_sideways.shape[0])
num_edges = len(node_indices) 
print(f"Total number of edges in the graph: {num_edges}")

adjacency_matrix_bear = compute_adjacency_matrix(adjacency_matrix_bear, epsilon=0.5)
node_indices, neighbor_indices = np.where(adjacency_matrix_bear == 1)
graph2 = GraphInfo(edges=(node_indices.tolist(), neighbor_indices.tolist()), num_nodes=adjacency_matrix_bear.shape[0])
num_edges = len(node_indices) 
print(f"Total number of edges in the graph: {num_edges}")
...
graph3 = GraphInfo(edges=(node_indices.tolist(), neighbor_indices.tolist()), num_nodes=adjacency_matrix_bull.shape[0])
num_edges = len(node_indices) 
print(f"Total number of edges in the graph: {num_edges}")
...
graph4 = GraphInfo(edges=(node_indices.tolist(), neighbor_indices.tolist()), num_nodes=adjacency_matrix_std.shape[0])
num_edges = len(node_indices) 
print(f"Total number of edges in the graph: {num_edges}")
